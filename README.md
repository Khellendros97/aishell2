...
云配置为**构建期常量**(`build.rs` 注入),客户端无修改入口;取值顺序为**环境变量 → 项目根 `release.env`(gitignore 本地文件)**:
...
# 方式二:项目根 release.env(gitignore 不入库,VS Code 集成终端等读不到环境变量时兜底)
...
npm run tauri dev                        # 开发模式(自动读 release.env,无需手动注入)
```

> 凭据纪律:client_secret 属于敏感凭据,只存在于环境变量 / release.env(不入库);回调地址白名单在云平台后台登记 `http://127.0.0.1:38901/auth/callback`(客户端本地回环监听,端口固定)。
