 ...
 *  scope 非空时替换检索步骤：只检索用户勾选的标签范围（#标签名 + 时间段），不做全量分析。 */
function timelineAnalysisPrompt(scope?: TimelineAnalysisScopeItem[]): string {
  const tail = [
    '2. 产出两类产物：',
  ...
  const lines = scope.map(
    (s) => `- 标签「${s.name}」：${fmt(s.fromTs)} ~ ${fmt(s.toTs)}（检索时 keyword 用 \`#${s.name}\`，fromTs=${s.fromTs}，toTs=${s.toTs}）`,
  );
  return [
    '请分析本项目的项目时间线中以下标签划定的范围（用户已勾选），生成项目日志与可重用 Skill。项目时间线记录了本项目近期的 SSH 连接、终端与远程命令及结果、文件上传下载、以及 AI 问答与工具调用历史（保留 30 天），用 timeline_search 工具检索。',
    '',
    '分析范围（只检索这些标签，不要做范围外的全量检索）：',
    ...lines,
    '',
    '步骤：',
    '1. 对每个标签分别用 timeline_search 检索：keyword 必须带对应的 `#标签名`（命中直接打标与标签对选区内的事件），并带上该标签的 fromTs/toTs；结果较多时在标签范围内换更精确的关键词分批细化。梳理出范围内完成的主要工作、关键命令与操作结果。',
    ...tail,
  ].join('\n');
}

/** 项目时间线分析（时间线标签页「AI 分析」按钮入口，仿 convertNoteToSkill 流程）：
 *  新建 AI 会话 → 输入区注入分析指令 → 标准 send()（含审批/标题等既有路径）。
 *  scope 为用户在时间线页勾选的标签范围；传入则只分析这些标签划定的时间段。 */
async function analyzeTimeline(scope?: TimelineAnalysisScopeItem[]): Promise<void> {
  const ctx = viewContext;
  if (!ctx || unmounted) { toast('AI 面板尚未就绪'); return; }
  await loadSessions();
  if (viewContext !== ctx || unmounted) { toast('AI 面板已离开当前页面'); return; }
  ...
  updateSendBtn();
  clearChips();
  input.appendChild(document.createTextNode(timelineAnalysisPrompt(scope)));
  input.focus();
  await send();
}
