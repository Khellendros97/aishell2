/**
 * 编辑器标签页(React 版)。
 * 对照 legacy/pages/workbench/editor.ts(原型 .proto/workbench-editor.js)逐条迁移:
 *   - 脏标记 ● / 800ms 防抖自动保存 / Ctrl+S 立即保存 / 关闭时静默落盘
 *   - 语法高亮 @codemirror/language-data 按文件名/扩展名匹配(不可识别为纯文本)
 *   - Ctrl+F / Ctrl+H 查找替换栏(中文 UI 顶部浮层,复用 @codemirror/search)
 *   - SFTP 打开的文件保存走 sftp_write,带打开时 stat 快照做「外部修改」冲突检测
 * 与后端的接口点:fs_read / fs_write / sftp_read / sftp_stat / sftp_write / staging_add /
 *   staging_list + fs:changed 事件订阅(本地文件被 AI write/edit 落盘后刷新同路径标签)。
 * 架构差异(相对 legacy,行为不变):
 *   - keep-alive:组件挂载时创建 EditorView、卸载时 destroy,active 仅切显隐
 *     (legacy 编辑器无 tab-activated 行为,故不设 [active] 效果);
 *   - 标题走 useWorkbench.getState().setTabTitle、AI 句柄 wbHandles.ai、事件 wbEvents;
 *   - 暂存警告条改为 React 状态渲染;右键菜单/查找栏/冲突弹窗仍为命令式 DOM;
 *   - 主题联动改为每实例挂载时订阅 onThemeChange、卸载退订(legacy 是模块级一次性订阅);
 *   - 外部入口:侧栏等模块直接 import 本文件导出的 openLocalFile / openRemoteFile
 *     (legacy 中对应 explorer.ts / sftp.ts 内联的 openTab 调用)。
 */
...
import {
  search, SearchQuery, getSearchQuery, openSearchPanel, closeSearchPanel, searchPanelOpen,
  setSearchQuery, findNext, findPrevious, replaceNext, replaceAll,
} from '@codemirror/search';
import type { Panel, ViewUpdate } from '@codemirror/view';
import {
  fsRead, fsWrite, onFsChanged, sftpRead, sftpStat, sftpWrite, stagingAdd, stagingList,
} from '../../../api';
import { copyText, showContextMenu, toast, uid, type CtxMenuItems } from '../../../ui';
import { icon, type IconName } from '../../../icons';
import type { FileRef } from '../../../types';
import {
  useWorkbench, wbEvents, wbHandles, tabApis, type Tab, type TabProps,
...
/* CodeMirror 主题随全局主题切换:两套高亮配色都显式重配。
   compartment 模块级共享(同 legacy),编辑器与只读视图(createReadonlyView)共用。 */
...
/** 按当前全局主题重配某个 CodeMirror 视图(编辑器标签与 diff 只读视图共用;供 DiffTab 使用) */
export function reconfigureTheme(view: EditorView): void {
  view.dispatch({ effects: cmTheme.reconfigure(cmThemeExt()) });
}

/** 共享主题 compartment 的初始扩展(NoteTab 等同主题编辑器挂载时用;
 *  之后全局主题切换由各实例的 onThemeChange → reconfigureTheme 联动) */
export function editorCmThemeExt(): Extension {
  return cmTheme.of(cmThemeExt());
}
...
interface EditorEntry {
  tab: Tab;
  /** 展示路径:本地为 tab.data.path,远端为 sftp.remotePath(api.getPath 用) */
  path: string;
  baseTitle: string;
  dirty: boolean;
  ...
  conflict: boolean;
  /** 冲突「仅提示一次」标记(legacy 用 !conflict 判断,因 flag 先置位而恒不触发,已修复) */
  conflictToastShown: boolean;
  /** 冲突弹窗防重入 */
  conflictDialogOpen: boolean;
  /** 远程文件是否已加入当前 AI 会话暂存;本地文件不使用。 */
  staged: boolean;
  /** staged 变化时通知组件重渲染警告条(React 状态,替代 legacy 的 classList 操作) */
  notifyStaged: (staged: boolean) => void;
}

/** 已打开的编辑器标签登记:fs:changed 外部刷新 + 冲突弹窗存活判断 */
...
function queueSave(entry: EditorEntry, silent: boolean, explicit = false): Promise<void> {
  ...
  const run = remote
    ? entry.chain.then(async () => {
        ...
        entry.conflictToastShown = false; // 冲突已解决,下次再冲突重新提示
...
function handleRemoteConflict(entry: EditorEntry, err: RemoteConflictError, explicit: boolean): void {
  if (!explicit) {
    // 修复 legacy bug:原实现用 !entry.conflict 判断「是否已提示」,但 queueSave 抛错前已把
    // conflict 置 true,导致自动保存路径永不提示;改为独立标记,首次检测提示一次。
    if (!entry.conflictToastShown) {
      entry.conflictToastShown = true;
  ...
  conflictDialog(entry.baseTitle, err.actualSize, err.actualMtime).then((choice) => {
    ...
    if (choice === 'overwrite') {
      ...
      entry.conflict = false;
      entry.conflictToastShown = false;
...
async function refreshRemoteStaging(entry: EditorEntry): Promise<void> {
  ...
  if (!sftp?.serverId || !sftp.remotePath || !projectId || !sessionId) {
    entry.staged = false;
    entry.notifyStaged(false);
    return;
  }
  try {
    const list = await stagingList(projectId, sessionId);
    entry.staged = list.some((item) => item.serverId === sftp.serverId && item.remotePath === sftp.remotePath);
  } catch {
    entry.staged = false;
  }
  entry.notifyStaged(entry.staged);
}
...
function addSelectionToAI(view: EditorView, path: string, from: number, to: number, content: string): void {
  ...
  if (wbHandles.ai?.addFileRef) {
    wbHandles.ai.addFileRef(ref);
...
/* ---------- 渲染器(React 版):组件挂载 = 创建视图,卸载 = destroy ---------- */

/** 在 root 容器内创建编辑器视图并组装运行状态(保持标签存活期内只建一次) */
function createEditorEntry(
  root: HTMLElement, tab: Tab, notifyStaged: (staged: boolean) => void,
): EditorEntry {
...
export function EditorTab({ tab, active: _active }: TabProps): JSX.Element {
  const rootRef = useRef<HTMLDivElement>(null);
  const entryRef = useRef<EditorEntry | null>(null);
  const [staged, setStaged] = useState(false);
  const remote = Boolean(
    (tab.data.sftp as { serverId?: string; remotePath?: string } | undefined)?.serverId
      && (tab.data.sftp as { serverId?: string; remotePath?: string } | undefined)?.remotePath,
  );

  /* keep-alive:CodeMirror 实例生命周期 = 组件挂载/卸载;
     标签数据(id/path/sftp)在标签存活期内不变,故只挂载一次 */
  useEffect(() => {
    const root = rootRef.current;
    if (!root) return;
    const entry = createEditorEntry(root, tab, setStaged);
    entryRef.current = entry;
    entries.set(tab.id, entry);

    /* 命令式句柄注册(旧版渲染器返回的 api 等价物,供外部模块按 tab id 获取) */
    tabApis.set(tab.id, {
      getPath: () => entry.path,
    ...
    const offStaging = wbEvents.on('staging-changed', () => {
    ...
    // tab 对象在标签存活期内 id/data 稳定,仅挂载一次
  ...
  return (
    <>
      {remote && (
        <div className={`ed-staging-warning${staged ? ' hidden' : ''}`}>
          <Icon name="alert" />
          <span>该远程文件尚未暂存，请在修改前右键暂存</span>
        </div>
      )}
      <div className="ed-root" ref={rootRef} />
    </>
  );
}

/* ---------- 外部入口(侧栏等模块直接 import;legacy 中为 explorer.ts / sftp.ts 内联的 openTab 调用) ---------- */
/** 本地文件在编辑器中打开:tab id = editor:<path>,同路径去重激活 */
export function openLocalFile(path: string, name?: string): Tab | null {
...
/** 远端文件在编辑器中打开:tab id = editor:sftp:<serverId>:<remotePath>;
 *  stat 为打开时远端属性快照(保存前「外部修改」冲突检测的基线) */
export function openRemoteFile(
  serverId: string, remotePath: string, name?: string,
  stat?: { size: number; mtime: number } | null,
): Tab | null {
  const display = name ?? remotePath.split('/').filter(Boolean).pop() ?? remotePath;
  return useWorkbench.getState().openTab({
    id: `editor:sftp:${serverId}:${remotePath}`,
    type: 'editor',
    title: display,
    data: { sftp: { serverId, remotePath, stat: stat ?? undefined }, name: display },
  });
}
