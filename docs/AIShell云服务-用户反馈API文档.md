...
| `reporterName` / `reporterDept` | 提交者姓名和部门快照；由服务端从当前登录会话写入 |
...
| 单个附件 | 每个文件 ≤ 20 MB（20 × 1024 × 1024 字节） |
| 总附件大小 | 同一条反馈所有附件合计 ≤ 100 MB（100 × 1024 × 1024 字节） |
| 允许类型 | 常见图片、PDF、Office 文档、文本/日志文件，以及 `zip` / `7z` / `rar` / `tar.gz` 压缩包 |
| 拒绝类型 | 脚本、可执行文件、SVG、HTML；即使伪装扩展名或修改 MIME 类型也会被拒绝 |

服务端按安全扩展名白名单校验文件名，并将声明的 MIME 类型作为下载元数据保存；不会据 MIME 类型放宽扩展名限制。允许的扩展名包括 `png` / `jpg` / `jpeg` / `gif` / `webp` / `bmp` / `ico` / `avif` / `heic`、`pdf`、`doc` / `docx` / `xls` / `xlsx` / `ppt` / `pptx` / `odt` / `ods` / `odp`、`txt` / `log` / `md` / `markdown` / `csv` / `tsv` / `json` / `xml` / `yaml` / `yml`，以及 `zip` / `7z` / `rar` / `tar` / `gz` / `tgz` / `bz2` / `xz` / `tar.gz`。`svg`、`html`、`htm`、脚本和可执行文件扩展名不在白名单中。
